...
import pandas as pd
from sisutils.msi_utils import (augment_dataset, plot_acc_loss, evaluate_ds, plot_cm, plot_roc, get_img_anthocyanines_nparray)
from sisutils.sis_model import (sis_msi_imgreg_model, sis_msi_imgcls_MobileNetV2_model)
...
CONFIG = dict (
    run = 6,
    model_arch_module = 'imgcls_mv2_cbwssam', # model: imgcls_mv2_cbwssam (T0-1*,T1-10*,T2-100*), NA (imgcls_mv2_cbam, imgcls_mv2)
    model_arch_type = 't0mvit_h4b4', # NA, (T0) t0mvit_h4b4, (T1) t1mvit_h4b4, (T2) t2mvit_h4b4
    num_labels = 5, # No. of Classes
    img_width = 128, # (hxw)
    img_height = 128,
    ...
    modelweight_parentdir = 'weight/imgcls/antregmsi128/',
    kerasweight_parentdir = 'weight/keras/antregmsi128/',
    model_logdir = 'metricinfo/antregmsi128/',
...
if __name__ == '__main__':
    ...
    exp_name='tdm_ant_' + str(CONFIG['model_arch_module']) + "_" + str(CONFIG['model_arch_type'])  + "_img_" + str(CONFIG['img_width']) + "_batch_"  + str(CONFIG['batch_size']) + "_run_" + str(CONFIG['run'])
    ...
    X_train, y_train = get_img_anthocyanines_nparray(CONFIG['img_dirpath'], CONFIG['band_config'], csv_file=train_csvfile, standard_scalar=CONFIG['standard_scalar'],
                                             minmax_scalar=CONFIG['minmax_scalar'], target_shape=(CONFIG['img_height'], CONFIG['img_width']))
    X_val, y_val = get_img_anthocyanines_nparray(CONFIG['img_dirpath'], CONFIG['band_config'], csv_file=val_csvfile, standard_scalar=CONFIG['standard_scalar'],
                                         minmax_scalar=CONFIG['minmax_scalar'], target_shape=(CONFIG['img_height'], CONFIG['img_width']))
    X_test, y_test = get_img_anthocyanines_nparray(CONFIG['img_dirpath'], CONFIG['band_config'], csv_file=test_csvfile, standard_scalar=CONFIG['standard_scalar'],
                                           minmax_scalar=CONFIG['minmax_scalar'], target_shape=(CONFIG['img_height'], CONFIG['img_width']))
    ...
    with strategy.scope():
        model = sis_msi_imgreg_model(input_shape=(CONFIG['img_height'], CONFIG['img_width'], CONFIG['channels']), model_arch_module=CONFIG['model_arch_module'],
                                                 model_arch_type=CONFIG['model_arch_type'], no_classes=CONFIG['num_labels'])
        '''
        model = sis_msi_imgcls_MobileNetV2_model(input_shape=(CONFIG['img_height'], CONFIG['img_width'], CONFIG['channels']), model_arch_module=CONFIG['model_arch_module'],
                                                 model_arch_type=CONFIG['model_arch_type'], no_classes=CONFIG['num_labels'])
        '''
        print("Model Summary:")
        model.build(input_shape=(None,CONFIG['img_height'], CONFIG['img_width'], CONFIG['channels']))
        print(model.summary())
        model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=CONFIG['learning_rate']), loss='huber_loss',
                      metrics=['mean_absolute_error', 'mean_squared_error', 'root_mean_squared_error'])

    # Model Fit with Batch Size
    history = model.fit(train_ds, validation_data = val_ds, epochs = CONFIG['epochs'], verbose = 1, batch_size = mgpu_batch_size, class_weight = CONFIG['class_weight'],
    ...
    print(classification_report(y_true=true_labels_test, y_pred=thres_preds_test, zero_division=0))
    print("\n")
